...
export default async (req, context) => {
    ...
    try {
        ...
        if (req.method === 'POST') {
            // Allow anon users to interact (login-free social media style)
            // If we can't track IP, we just can't enforce strict rate limits or persistence across sessions for them
            // but we shouldn't block the interaction entirely.
